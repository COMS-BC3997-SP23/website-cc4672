#!/usr/bin/env python
# coding: utf-8
# This is the code from a very guided project, see how step by step it is?
# Original Website: Datacamp
...
get_ipython().run_cell_magic('sql', '', '-- Select year, an average of user_score, and a count of games released in a given year, aliased and rounded\n-- Include only years with more than four reviewed games; group data by year\n-- Order data by avg_user_score, and limit to ten results\n\nSELECT game_sales.year, \n       ROUND(AVG(reviews.user_score), 2) AS avg_user_score, \n       COUNT(*) AS num_games\nFROM game_sales\nJOIN reviews ON game_sales.game=reviews.game\nGROUP BY game_sales.year\nHAVING COUNT(*) > 4\nORDER BY avg_user_score DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', '\n-- Select the year results that appear on both tables\nSELECT tum.year\nFROM top_user_years_more_than_four_games tum\nINNER JOIN top_critic_years_more_than_four_games tcm\nON tum.year = tcm.year;')
